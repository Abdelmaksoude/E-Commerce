    ...
    <div class="row px-xl-5">
        ...
        <div class="col-lg-9 col-md-8">
            <div class="row pb-3">
                ...
                @foreach ($all_product_for_this_product as $product_category)
                    <div class="col-lg-3 col-md-4 col-sm-6 pb-1">
                        <div class="product-item bg-light mb-4">
                            <div class="product-img position-relative overflow-hidden">
                                <img class="img-fluid w-100" style="max-height: 300px" src="{{asset('Attachments/Product/'.$product_category->photo)}}" alt="">
                                <div class="product-action">
                                    <form class="addToCartForm" id="addToCartForm_{{ $product_category->id }}" enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" value="{{ $product_category->id }}" name="id" >
                                        <input type="hidden" value="{{ $product_category->title }}" name="title">
                                        <input type="hidden" value="{{ $product_category->final_value }}" name="price">
                                        <input type="hidden" value="{{ $product_category->photo }}"  name="image">
                                        <input type="hidden" value="1" name="quantity">
                                        <button data-product-id="{{ $product_category->id }}" class="btn btn-outline-dark btn-square addToCartBtn"><i class="fa fa-shopping-cart"></i></button>
                                    </form>
                                    @auth
                                        @if (App\Models\FavouriteProduct::where('product_id', $product_category->id)->where('user_id', Auth::user()->id)->exists())
                                            <a class="delete-favourite btn btn-outline-dark btn-square addFavourite" data-id="{{ $product_category->id }}"><i class="fa fa-heart"></i></a>
                                        @else
                                            <a class="add-favourite btn btn-outline-dark btn-square addFavourite" data-id="{{ $product_category->id }}"><i class="far fa-heart"></i></a>
                                        @endif
                                    @endauth
                            ...
                            <div class="text-center py-4">
                                <a class="h6 text-decoration-none text-truncate" href="{{ route('details',$product_category->id) }}">{{ $product_category->title }}</a>
                                <div class="d-flex align-items-center justify-content-center mt-2">
                                    <h5>{{ $product_category->final_value }}$</h5><h6 class="text-muted ml-2"><del>{{ $product_category->price }}</del></h6>
                                </div>
